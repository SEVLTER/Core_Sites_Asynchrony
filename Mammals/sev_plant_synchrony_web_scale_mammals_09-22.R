...
# calculate totmass sd, mean, CV 2009 - 2022
totmass_cast$sd<-apply(totmass_cast[,19:32], 1, sd, na.rm=TRUE)
totmass_cast$mu<-apply(totmass_cast[,19:32], 1, mean, na.rm=TRUE)
...
# calculate the number of years (max = 14)
totmass_cast$year_count<-rowSums(!is.na(totmass_cast[,19:32]))
...
# filter to 2009 onwards
mass_species_core<- mass_species_core %>% filter(year>2008)
...
web_species_cast$sum<-rowSums(web_species_cast[,10:23],na.rm=T)
...
web_species_stability$years_count<-rowSums(!is.na(web_species_stability[,10:23]))
...
web_species_stability$species_sd<-apply(web_species_stability[,10:23], 1, sd, na.rm=TRUE)
web_species_stability$species_mu<-apply(web_species_stability[,10:23], 1, mean, na.rm=TRUE)
...
summary(msrs)
write.csv(msrs, "webscale_mass_richness_stability_mammals_09-22.csv")
...
species_async <- species_async[,1:23]
# there were NO webs that had only 1 species through time

# pivot to long format
SEV_plant <- pivot_longer(species_async, cols = '2009':'2022', names_to = "year", values_to = "abundance")
...
write.csv(async_stabil_spring, "async_stabil_spring_allwebs_mammals.csv")
...
write.csv(asynchrony_stability, "asynchrony_stability_allwebs_mammals.csv")
