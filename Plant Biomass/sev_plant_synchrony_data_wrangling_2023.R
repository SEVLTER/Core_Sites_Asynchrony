...
### J Rudgers August 2023, revised Oct 2023
rm(list=ls(all=TRUE)) 
...
# possible number of years (to 2022) = 24
...
species <- as.data.frame(msrs %>% filter(kartez!="totmass" & kartez!="richness" & kartez!="shannonH" & kartez!="evenness")
                         %>% filter(years_count>6))
write.csv(species, "plant_species_stability.csv") #34,891 obs
...
#write.csv(async_stabil_spring, "asynchrony_stability_spring_allquads.csv")
...
#write.csv(async_stabil_fall, "asynchrony_stabil_fall_allquads.csv")
...
write.csv(asynchrony_stability, "asynchrony_stability_allquads.csv")

################### merge data with plant traits data ----------------------------
summary(species$kartez) #322 species, 1816 quadrats
# import plant traits
traits<-read.csv("C:/Users/jrcassvc/Desktop/SEV Data/Trait Data/sev_all_traits_2023.csv",stringsAsFactors = T)
stability_traits <- merge(species,traits, by.x="kartez", by.y="kartez", all.x=F) 
#27,267 observations of species in quadrats with trait data included
summary(stability_traits)
write.csv(stability_traits, "plant_species_stability_traits.csv")
